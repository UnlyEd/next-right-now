...
module.exports = withBundleAnalyzer(withSourceMaps({
  ...
  webpack: (config, { isServer, buildId }) => {
    ...
    if (isServer) { // Trick to only log once
      console.debug(`[webpack] Building release "${APP_VERSION_RELEASE}" using NODE_ENV="${process.env.NODE_ENV}" ${process.env.IS_SERVER_INITIAL_BUILD ? '| IS_SERVER_INITIAL_BUILD="1"' : ''}`);
    }
