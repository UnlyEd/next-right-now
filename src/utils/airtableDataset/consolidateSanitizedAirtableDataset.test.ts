...
import consolidateSanitizedAirtableDataset from './consolidateSanitizedAirtableDataset';
 ...
 */
describe(`utils/airtableDataset/consolidateSanitizedAirtableDataset.ts`, () => {
  beforeEach(() => {
    global.console = global.muteConsole();
  });

  describe(`consolidateSanitizedAirtableDataset`, () => {
    describe(`should resolve relationships from a sanitized airtable record`, () => {
      describe(`when using the "simple" customer (simple relationships, no nested relationships)`, () => {
        ...
        const consolidatedDataset: SanitizedAirtableDataset = consolidateSanitizedAirtableDataset(mockedAirtableRemoteSchemaFull, sanitizedAirtableDataset);
        const consolidatedCustomer: AirtableRecord<Customer> = <AirtableRecord<Customer>>find(consolidatedDataset, { __typename: 'Customer' });
        ...
        test(`should contain expected "campuses"`, async () => {
          ...
          expect(consolidatedCustomer?.campuses).toMatchObject([{
            __typename: 'Campus',
            ...
            place: {
              ...
              label: 'FULL place 1 (multioffices)',
              address: null,
            },
          }]);
          expect(consolidatedCustomer?.campuses[0].educationalProgramSteps).toBeArrayOfSize(1);
          expect(consolidatedCustomer?.campuses[0].educationalProgramSteps[0].campuses[0].place.label).toEqual('FULL place 1 (multioffices)');
        });
